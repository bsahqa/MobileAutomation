...
namespace MobileAutomation.PageObjects
{
    abstract class BasePage
    {
        ...
        public static string GetElementText(SeleneElement elements)
        {
            ...
            {
                return elements.With(timeout: 30).GetAttribute("value");
            }
            else
            {
        ...
        public virtual SeleneElement ScrollDownToElements(string accessibilityId)
        {
            ...
            else
            {
                ...
                // iOS specific scroll logic to scroll down until the element is found
                Dictionary<string, object> scrollObject = new Dictionary<string, object>
        {
            { "direction", "down" },
            { "element", elem.ActualWebElement },
            { "toVisible", true }  // Scroll until the element becomes visible
        };

                while (!elem.Matching(Be.Visible))
                {
                    ((IJavaScriptExecutor)MobileDriver.Driver).ExecuteScript("mobile: scroll", scrollObject);
                }
